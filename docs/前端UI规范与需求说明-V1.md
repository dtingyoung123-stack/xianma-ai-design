...
本次修订（V1.2）依据平台内置的 UI 规范页（顶部右侧「UI 样式」入口，对应 `src/app/ui-guide/page.js`）与设计令牌文件 `src/app/globals.css`，将全站交互主色统一为先马山茶红，并明确品牌色与语义状态色的使用边界。

## 0. 当前状态

- 本文档基于当前已调整完成的原型页面与内置 UI 规范页整理，适合作为前端开发实现依据。
- 2026-08-14 已确认将全站交互主色切换为山茶红；该状态不代表已部署或已上线。
...
仅用于品牌交互、主操作、选中态和焦点态。大面积背景保持白色或浅中性色，不用于表达失败、删除或告警。

| 名称 | 变量 | 取值 | 用途 |
| --- | --- | --- | --- |
| Primary | `--brand-primary` | `#D9353F` | 主按钮、选中态、强调 |
| Primary Hover | `--brand-primary-hover` | `#C22F38` | Hover 加深 |
| Primary Active | `--brand-primary-active` | `#A92831` | 按下态 |
| Primary Deep | `--brand-primary-deep` | `#A92831` | 深色强调 |
| Primary Soft | `--brand-primary-soft` | `rgba(217,53,63,0.08)` | 选中背景、标签底 |
| Primary Border | `--brand-primary-border` | `rgba(217,53,63,0.15)` | Soft 按钮、选中卡片描边 |
...
语义色仅用于表达操作结果和状态反馈，配对使用「前景色 + 浅底色」，不得被品牌主题色覆盖。由于品牌主色与危险色同属红色系，危险操作必须同时使用明确文案或警示图标。
...
- `--focus-ring`：`0 0 0 3px rgba(217,53,63,0.14)`，所有可聚焦控件 `:focus-visible` 统一使用。
...
- 高 40px，圆角 8px，`--border-base` 边框，Focus 使用 `--brand-primary`。选项较多时的单选（模型切换、分类筛选）。
...
- 页面级 Tabs：底部山茶红下划线选中态（`--brand-primary`）。

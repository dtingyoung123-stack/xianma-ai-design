...
- Push in this fixed order: run `git push origin <branch>`; on a connection reset or timeout, retry once with `git -c http.version=HTTP/1.1 push origin <branch>`; if that retry also fails while `api.github.com` is reachable, immediately use the verified GitHub Git Database API fast-forward fallback. Do not add SSH or repeated network probes to this path.
- In the API fallback, read Base64 blob bytes from the committed Git objects with `git cat-file`, never from working-tree text; create and verify blobs and the tree against local Git SHAs, verify the expected remote parent, create the commit, then update the branch only as a non-force fast-forward. If API serialization changes only terminal commit-message bytes, align the local commit object once to the returned API object instead of retrying amend/date variants. Never print credentials, and finish by verifying remote `main`, local `HEAD`, and `origin/main` are identical.

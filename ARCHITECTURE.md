...
- 商品主状态由底层状态映射为前端合并状态：待完善（草稿、导入失败、信息待确认/补充）、处理中（识别中、还原中）、待确认（候选输出、部分成功）、可用（已确认）、修订中和已归档；纠错提交记录保留为事件，但不作为独立商品主状态。
- 商品学习的运营闭环为“AI 输出完成 → 待确认 → 运营确认进入可用，或提交纠错进入处理中后重新输出，或删除未发布且未被下游使用的个人商品”；仅打开/取消圈选纠错不改变商品状态。

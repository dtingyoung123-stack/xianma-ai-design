...
提示词库页使用“团队提示词 / 我的提示词 / 灵感广场”三个内容域：团队提示词仅展示当前用户有组织可见权限且已审核通过的内容；我的提示词承载手动创建、历史沉淀和外部收藏；灵感广场承载外部同步数据，不等同于公司已验证资产。提示词是文本资产，不使用封面图或素材式图片网格；页面采用单列紧凑文本列表，直接展示名称、正文摘要、用途分类、标签、来源和状态。页面只保留“用途分类”一个业务分类维度，选项为通用、商品主图、详情页、买家秀、场景图、营销海报和短视频；不向用户展示或要求维护适用工具与适用模型，同类型模型默认均可使用，模型差异只影响生成效果、不作为使用限制。页面保留搜索和用途分类筛选，复制作为一期最高频主操作；收藏、编辑、提交团队、取消提交、审批和删除按内容域、状态及身份显示。普通用户提交时默认进入本人所属钉钉部门审批，不手动选择组织；部门管理员自动处理授权组织的提示词入库，审批员处理所选事项与组织范围内的申请，系统管理员同时管理全部提示词和外部来源。提示词不再作为素材库文件类型；工具关联和已验证模型只作为后端可选扩展字段预留，后续有明确的工具回填协议或模型效果验证机制后再开放，不纳入一期用户录入与筛选。

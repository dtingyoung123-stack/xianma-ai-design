...
先马AI内容与视觉平台默认采用“白色/半透明白面板 + 细描边 + 柔和阴影”的工具台风格。不要再使用“默认卡片无阴影、hover 才加阴影”的旧规则。只有表格行、列表行、极小控件可只用描边不用阴影。
...
| `/admin/permission` | 权限管理 | `src/app/admin/permission/page.js` | 开发中 |
| `/admin/models` | 模型管理 | `src/app/admin/models/page.js` | 前端交互原型 |
| `/admin/settings` | 系统设置 | `src/app/admin/settings/page.js` | 原型中 |
| `/points` | 积分中心 | `src/app/points/page.js` | 前端交互原型 |

使用 `PageShell` 的页面状态来源于 `src/config/navigation.js` 的 `routeMeta`；使用独立 `WorkbenchShell` 的页面需同步传入相同业务状态。`已上线` 仅用于业务已确认正式可用的功能；`开发中` 表示需求和原型已进入开发阶段但尚未上线；`原型中` 表示仍处于交互验证阶段；`可用` 仅表示路由已接入。

权限管理页使用“人员平铺分页列表 + 角色说明弹窗 + 角色下拉多选 + 审批事项多选 + 弹窗组织树”：角色权限表默认不占用页面空间，在“管理权限人员”标题旁显示帮助图标，点击后弹窗按系统管理员、部门管理员、审批员、普通用户顺序展示查看数据、审批、资源操作、管理页面和获得方式。列表展示全部同步人员，用于搜索、筛选和统一“配置”操作，不展示“权限来源”列；桌面端列表合并为人员/部门、身份/状态、平台角色、组织数据范围、审批权限、操作六列，在 1280px 视口内不出现内部横向滚动。普通用户作为固定基础角色，钉钉主管是独立同步标识，一期默认叠加部门管理员和审批员，两项均可分别取消且不影响其主管数据范围。部门管理员自动拥有指定组织团体资源的编辑、删除及素材/提示词入库审批；审批员显示资源审批事项多选和组织范围，至少各选一项才能保存。积分申请审批一期仅系统管理员拥有，在非系统管理员配置中显示为不可选。系统管理员固定显示全部数据和全部审批，不受众创事业群授权范围限制。平台角色和审批事项多选支持点击控件外部自动收起，点击下拉内部勾选时保持展开。授权组织树只展示众创事业群及其任意层级下级组织，众创事业群根节点可选择；支持名称或完整路径搜索、展开收起、父组织包含下级和部分下级半选状态，默认展开人员所属组织及已授权组织所在分支。页面不展示组织人数，不提供组织新增、移动、删除或主管维护能力，后台管理导航只对系统管理员渲染。

素材库页使用“范围分栏 + 筛选工具条 + 响应式素材卡片网格”：个人、团体和公共素材位于同一页面，保留搜索、类型筛选、上传、查看、编辑、下载、删除和分页。页面 UI 骨架以提示词库为标准：一级范围使用灰底分段 Tab，团体素材的“全部 / 待审核”使用下划线二级切换，搜索、类型下拉和结果数量组成同一行筛选区，按钮、空状态、卡片描边/阴影和顶层弹窗遵循相同视觉规则；素材仍保留 `4:3` 图片、视频和音频卡片网格，不改为文本列表。个人素材卡片将“提交到团队”作为主要协作操作；部门管理员、具备素材审批事项的审批员和系统管理员在页面顶部始终可见“待我审批（数量）”入口，点击直接进入本人组织范围内的待审核列表。审核中的个人素材只能先取消提交，再编辑或删除。普通查看者只显示查看、下载和选择使用，管理按钮按资源范围和当前角色渲染。上传、编辑、详情、审核和删除均使用顶层响应式弹窗，可见组织只允许选择众创事业群及其下级，并延续父组织包含下级和搜索规则。

素材库上传弹窗对新上传图片增加页面局部的自动识别命名流程：文件列表按图片展示缩略图、原文件名、识别状态、建议名称/类目/标签、单张采纳和可编辑最终字段；列表顶部提供“全部采纳”，但不得覆盖失败图片和已人工修改图片。每张图片只展示一套最终字段，不提供批量默认类目或标签；识别失败时保留原文件名、默认“通用”类目和空标签并允许继续上传。视频和音频保持紧凑文件行、不参与识别，只有批次包含视频或音频时才展示其统一类目和统一标签；素材范围、个人分组、统一备注和权限结构不变。弹窗沿用 `Modal` 的 Portal、固定头尾和内容区滚动，在桌面、1280px、1024px 与 390px 视口内不得横向溢出；该主动上传能力保留在 `MaterialsClient.jsx` 页面内，不登记为公共组件。
...
当前公共组件按真实目录分为全局组件、基础 UI、AI 工作台三层，共 24 个组件文件：
